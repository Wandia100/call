					...
					<?php  
						 if(isset($_POST['submit'])){
							...
							if(move_uploaded_file($_FILES["pic"]["tmp_name"], $target_file)) {
									include('connect.php');
									$sql1 = "SELECT * FROM counsellor WHERE userid='".$_POST["userid"]."' OR email= '" . $_POST["email"] . "' ";
	              					$result = $conn->query($sql1);
	              					if($result->num_rows > 0){
	              						 echo "<script>alert('Sorry, Userid or E-mail already exist!');</script>";
	              					}
	              					else{
									$sql = "INSERT INTO counsellor (name,address,contact,email,expertise,userid,password,pic)
